...
func (s *Service) Register(ctx context.Context, in RegisterInput) (*Session, error) {
	...
	err = s.tx.RunInTx(ctx, func(ctx context.Context) error {
		...
		return s.joinOnRegister(ctx, session, in.InviteCode)
	})
...
// joinOnRegister joins the group behind an invite code given at sign-up.
// Joining is idempotent, so replays end up with the same membership.
func (s *Service) joinOnRegister(ctx context.Context, session *Session, code string) error {
	code = strings.TrimSpace(code)
	if code == "" || s.joiner == nil {
		return nil
	}
