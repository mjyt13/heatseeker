...
10 с (realtime — W2-0). **Уведомления (2026-09-23) — сделаны.** `notifications`/`notifier_cursors`/`notification_prefs`/
`notification_mutes`/`notification_settings`/`notification_deliveries` (миграция 00012); журнал
группы читает `notify:fanout` от курсора с паузой `NOTIFY_DELAY_SEC` и пишет по строке на
участника с `dedupe_key` (повтор безопасен, история старой группы не переигрывается, D50); кто
уже прочитал сообщение — уведомления не получает, несколько материалов подряд склеиваются;
получатели по D48/D49 (предметы и общий чат — всем, материалы и задачи — участникам обсуждения и
автору, ответ мне — всегда); настройки по типам в группе, push и тихие часы по часам участника,
mute предмета/обсуждения/типа/группы всегда с концом; доставка — Expo push
(`internal/adapters/push`, `PUSH_PROVIDER`) с журналом доставок, в Expo Go на Android пуша нет.
Mobile: экран «Уведомления» (колокольчик с бейджем на главной, раздел в «Ещё»), экран настроек,
кнопка «приглушить» в обсуждении, регистрация push-токена и переход по нажатию на пуш.
**Напоминания и объявления (2026-09-24) — сделаны**, этап 2 закрыт: `announcements` и `reminders`
(миграция 00013). Личное напоминание (заголовок, заметка, время, повтор `NONE|DAILY|WEEKLY|MONTHLY`,
привязка к задаче/материалу/занятию) сканируется раз в минуту (`reminders:scan`) и уходит владельцу
напрямую, мимо журнала группы и mute (D51), кнопки «Отложить / Готово»; объявление старосты
(`announcement.send`, L2) — событие в журнал, уведомление всей группе, `pinned` держит наверху,
`urgent` выделяет объявление и пробивает тихие часы только у тех, кто сам это разрешил
(`urgent_in_quiet`, по умолчанию выключен, D51). Mobile: экраны «Объявления» и «Напоминания» в «Ещё».
Не сделано в этапе: markdown в сообщениях пока показывается текстом.

**Этап 3 (расписание) — начат (2026-09-22).** Готово:
...
форма (`class/edit`: новое, серия, с даты, одно занятие). Изменения расписания приходят уведомлением
(`SCHEDULE_CHANGED`, 2026-09-23). Не сделано в этапе: треды занятий (`LESSON`), запросы на
изменение и их утверждение. Заодно — уменьшенные
...
Запуск проверен в Expo Go на Android и в вебе (2026-09-17). `apps/mobile/eas.json` с профилями
`development` / `preview` / `production` и задачи `task mobile:build:dev|apk` есть (2026-09-26);
сам проект EAS (`eas login`, `eas init`, ключ FCM V1) заводится руками — порядок в README,
раздел «Проверить уведомления». Не сделано в этапе 0: оптимизирующий плагин Tamagui. Все принятые решения — в `docs/DECISIONS.md`,
...
task mobile:build:dev                             # EAS: APK с dev-клиентом (нужен для пуша; см. README)
task mobile:build:apk                             # EAS: обычный APK для группы (profile preview)
...
- `docs/DECISIONS.md` — лог принятых решений (D1–D51).
