...
| materials | `GET /groups/:id/materials?subject&tags&kind&q&cursor`, `GET /materials/:id`, `GET /materials/:id/open` → `{mode, drive_web_view_link?, stream_url?, s3_url?}`, `GET /materials/:id/stream` (прокси с Range), `GET /materials/:id/thumbnail?token=` (уменьшенная копия картинки, D47), `POST /groups/:id/materials/uploads` (presigned / прямой при `local`; `task_id` + `task_only` — сразу в задачу, D43), `POST …/complete`, `PATCH`, `/archive`, `/restore`, `DELETE`, `GET /groups/:id/materials/inbox`, `POST /materials/:id/classify`, `POST /materials/:id/share` (файл из задачи — группе, D43), `POST /materials/:id/publish-drive` (староста публикует загрузку на Диск позже или повторяет неудачную публикацию) |
...
| schedule | `GET /groups/:id/schedule?from&to` (занятия окна, до 190 дней), `POST /groups/:id/schedule/events` (разовое или серия, `client_id`), `GET/PATCH/DELETE /schedule/events/:id` (`version`; `scope=ALL\|FOLLOWING` + `from` — «с этого дня»), `GET/PUT/DELETE /schedule/events/:id/occurrences/:date` (одно занятие: отменить, изменить, вернуть), `GET /groups/:id/schedule/calendar-link` → личная ссылка на `GET /groups/:id/schedule.ics?token=` (D46); change-requests — вторая половина этапа 3 |
...
|---|---|---|
| **A. Через сервисный аккаунт** (в MVP, D32): файл → наше хранилище → задача `drive:upload` → SA создаёт файл в папке предмета (или в корне) с `description="Загрузил: <имя>"` и `appProperties` (ссылка на материал — синк не создаёт дубль) | SA — Редактор папки (D29) | у SA нет своей квоты: работает только в общем диске (Shared Drive); для «Моего диска» — открытый вопрос №6 |
...
Google Sign-In нужен только тем, кто выберет способ B. Настройка сервисного аккаунта —
