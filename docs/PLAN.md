...
MVP) `| INVITE | APPROVAL` — фундамент под закрытую схему.
...
Владелец общей папки выдаёт доступ **Читатель** (для LINK/CACHE/IMPORT) или **Редактор**
(если публикуем на Диск от имени сервиса, §6.3) e-mail'у сервисного аккаунта. Никаких
OAuth-экранов для студентов, верификации приложения Google не нужно, токены не протухают.
...
|---|---|---|
| **A. Через сервисный аккаунт** (рекомендуется): файл → наш API → SA создаёт файл в папке с `description="Загрузил: <имя>"` | SA — Редактор папки | файлы принадлежат SA → его квота 15 GB (в Shared Drive — квота организации) |
...
Google Sign-In нужен только тем, кто выберет способ B. См. [`OPEN-QUESTIONS.md`](OPEN-QUESTIONS.md) №1–2.
...
- Язык — Python (FastAPI) рекомендуется: экосистема локальных моделей (ollama/vLLM/llama.cpp
  через OpenAI-совместимый API, `sentence-transformers`, `faster-whisper` для возможной
  транскрибации записей лекций). Альтернатива — Go (единый язык репо); решается при старте W2-B.
